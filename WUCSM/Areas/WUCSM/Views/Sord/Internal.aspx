...
<asp:Content ID="Content2" ContentPlaceHolderID="MainContent" runat="server">   
    <div id="sordmainpanel" class="mini-panel" title="" style="width: 100%;" bodystyle="min-height:450px;"
        ...
        <div id="toolbar1" property="toolbar" style="padding: 2px;">
            ...
            &nbsp;&nbsp;
            <a class="mini-button" style="display:none" href="<%=Url.RouteUrl(new { controller="Sord",action="OrderOper" ,area="WUCSM"}) %>" >单据处理</a>
            ...
            //<p>障碍描述：' + objs + '</p>
                mini.showMessageBox({
                    ...
                    callback: function (action) {
                        ...
                        //判断概况描述是否为空
                        //mini.alert("");
                        //if (fault != null || fault != undefined || fault != "") {
                        // mini.alert(1);
                        //                                RequestAjax('<%=Url.RouteUrl(new { controller="Sord",action="MoveNextStt" ,area="WUCSM"}) %>', '跳转中', { ordno: ordno, stt: nextstt, fault: fault }, function () {
                        //                                    mini.showMessageBox({
                        //                                        title: "提示", iconCls: "mini-messagebox-info", buttons: ["ok"],
...
//            $.ajax({
//                url: "/Sord/RevisitSord",
//                type: "post",
//                data: { t: new Date(), ordno: ordno, RevisitStt: RevisitStt },
//                dataType: "json",
//                success: function (action) {
//                    if (action.msg == 'OK')
//                    { RefreshData(); } else {
//                        mini.alert(action.mes);
//                    }
//                },
//                error: function () {
//                    mimi.alert("系统异常!")
...
//            ahtml = ahtml + '障碍概述:';
//            ahtml = ahtml + '</td>';
//            if(item.FAULT!=undefined||item.FAULT!=null)
//            {
//                ahtml = ahtml + '<td style="width:245px;">';
//                ahtml = ahtml + item.FAULT;
//                ahtml = ahtml + '</td>';
//            }
            ahtml = ahtml + '</tr>';
            ...
            ahtml = ahtml + '<div>';
            if (item.STT != '-1') {
                ahtml = ahtml + '<a class="mini-button" onclick="UpdateSord(' + item.ORDNO + ',\'' + item.ISSYS + '\');">编辑</a> ';
                ahtml = ahtml + '<a class="mini-button" onclick="replysd(' + item.ORDNO + ');">回复</a> ';
                ahtml = ahtml + '<a class="mini-button" style="' + (item.STT != '0' ? 'display:none;' : 'color:green;') + '" onclick="startsord(' + item.ORDNO + ');">受理</a>';
                ...
                if (item.STT != '-1' && item.STT != '90') {
                    var name = item.TracerStt == 1 ? '取跟' : '跟踪';
                    ahtml = ahtml + '<a style="color:red" class="mini-button" stt=' + item.TracerStt + ' onclick="TracerSord(' + item.ORDNO + ',' + item.TracerStt + ',\'' + name + '\');">' + name + '</a> ';
                }
                //                ahtml = ahtml + '<a class="mini-button" onclick="replysd(' + item.ORDNO + ');">回复</a> ';\'' + item.TracerStt == 1 ? '取跟' : '跟踪' + '\'

                if (item.STT != '-1' && item.RevisitStt != 1 && item.STT == '90' || item.STT == '50') {
                    ahtml = ahtml + '<a style="color:red;" class="mini-button" onclick="RevisitSord(' + item.ORDNO + ',' + 1 + ');">回访</a> ';
                }
            }

            if (item.ISSYS == 'T') {
                ahtml = ahtml + '<a class="mini-button" style="' + (item.STT != '0' ? 'display:none;' : 'color:green;') + '" onclick="DeleteSord(' + item.ORDNO + ');">删除</a> ';
                
            }
            if (item.ISSYS == 'F' && item.STT != '-1') {
